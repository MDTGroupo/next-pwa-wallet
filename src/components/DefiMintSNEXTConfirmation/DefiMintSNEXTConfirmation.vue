<template>
    <div class="defi-mint-snext-confirmation">
        <tx-confirmation
            v-if="hasCorrectParams"
            :tx="tx"
            send-button-label="Submit"
            password-label="Please enter your wallet password to mint sNEXT"
        ...
        >
            <h2 class="not-visible" data-focus>Mint sNEXT - Confirmation</h2>
...
export default {
    name: 'DefiMintSNEXTConfirmation',
    ...
    methods: {
        async setTx() {
            ...
            this.dAmountDelegated = this.$fWallet.WEIToNEXT(this.d_amountDelegated);
        ...
        onSendTransactionSuccess(_data) {
            if (!this.isView) {
                this.$emit('change-component', {
                    to: 'staking-mint-snext-confirmation-success-message',
                    from: this.compName,
                    data: {
                ...
                this.$router.replace({
                    name: `defi-mint-snext-transaction-success-message`,
                    params,
                });
            }
        },
        ...
        onChangeComponent(_data) {
            let transactionRejectComp = `defi-mint-snext-transaction-reject-message`;
