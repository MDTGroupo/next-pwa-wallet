<template>
    <div class="view-defi-manage-collateral">
        <h1 class="with-back-btn"><f-back-button :route-name="backButtonRoute" /> Lock/Unlock {{ wnextTokenSymbol }}</h1>

        <h2 class="perex">
            Lock {{ wnextTokenSymbol }} to increase the collateral ratio and mint fUSD, unlock
            {{ wnextTokenSymbol }} after you repaid fUSD.
        </h2>
        ...
        <div class="grid">
            <div>
                <div class="df-data-item smaller">
                    ...
                    <div class="value"><f-token-value :token="wnextToken" :value="availableNEXT" /></div>
                </div>
                <div class="df-data-item smaller">
            ...
            <div class="defi-price-input-col align-center">
                <div class="defi-price-input">
                    ...
                    <div class="token-label">{{ wnextTokenSymbol }}</div>
                    <div class="collateral-info">
                        <div class="collateral-info-label">Unlock {{ wnextTokenSymbol }}</div>
                        <icon data="@/assets/svg/angle-double-left.svg" width="66" height="66" aria-hidden="true" />
                    </div>
                    <div class="collateral-info increase">
                        <div class="collateral-info-label">Lock {{ wnextTokenSymbol }}</div>
            ...
            <f-message v-else-if="increasedCollateral > 0" type="info" role="alert" class="big">
                ...
                <span class="inc-desc-collateral">
                    <f-token-value :token="wnextToken" :value="increasedCollateral" no-currency /> {{ wnextTokenSymbol }}
                </span>
                to your collateral
            </f-message>
            <f-message v-else-if="decreasedCollateral > 0" type="info" role="alert" class="big">
                You’re removing
                <span class="inc-desc-collateral">
                    <f-token-value :token="wnextToken" :value="decreasedCollateral" no-currency /> {{ wnextTokenSymbol }}
                </span>
...
*/
        ...
        _maxCollateral() {
            let max = this.collateral + this.availableNEXT - 1;
    ...
    methods: {
        ...
        formatInputValue(_value) {
            return parseFloat(_value).toFixed(this.$defi.getTokenDecimals(this.wnextToken));
        },

        updateMessage() {
            if (this.availableNEXT <= 0.01) {
                this.message = `Deposit more ${this.wnextTokenSymbol} to increase your collateral`;
            } else {
                this.message = '';
            }
