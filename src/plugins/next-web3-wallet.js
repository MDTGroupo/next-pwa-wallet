...
// Next web3 wallet plugin for VUE, based on https://github.com/Next-foundation/next-web3-wallet
export class NextWeb3Wallet {
    static install(_Vue, _options) {
        fWallet = new NextWeb3Wallet(_options);
    ...
    async getTransactionToSign({ from, to, value, memo = '' }) {
        ...
        return {
            ...
            _fee: !error ? this.WEIToNEXT(this.getTransactionFee(gasPrice, gasLimit)) : 0,
        };
    }
    ...
    */
    ...
    /**
     * Get the remaining balance (in NEXT) after deducting transaction fee.
     *
     * @param {*} _balance
     * @param {*} _gasPrice
     * @param {string} [_gasLimit]
    ...
    getRemainingBalance(_balance, _gasPrice, _gasLimit) {
        ...
        return parseFloat(this.WEIToNEXT(balance.sub(fee.mul(this.toBN(2)))));
    }
    ...
    getMaxRemainingBalance(_balance, _gasPrice) {
        ...
        return parseFloat(this.WEIToNEXT(balance.sub(fee.mul(this.toBN(1)))));
    }
    ...
    /**
     * @param _address
     * @param {('next' | 'ethereum' | 'binance')} _blockchain
     * @return {boolean}
     */
    isValidAddress(_address, _blockchain = 'next') {
        if (_blockchain === 'next' || _blockchain === 'ethereum') {
