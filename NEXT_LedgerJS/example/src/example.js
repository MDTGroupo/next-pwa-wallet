...
import NextNano from "../../lib/next-nano";
 ...
 * it into the console
 *
 * @param {NextNano} bridge
 * @returns {Promise<void>}
 */
async function showVersion(bridge) {
    // inform
    console.log("\nRequesting Next Nano Ledger app version.");

    // get the version info
    const ver = await bridge.getVersion();
    console.log(`    > Next Nano app version is: ${ver.major}.${ver.minor}.${ver.patch}`);
...
async function getSignedTransaction(bridge) {
    ...
    const tx = {
        ...
        value: "0xde0b6b3a7640000", /* 1 NEXT in WEI (1e+18) in HEX */
...
async function run() {
    // inform
    console.log("-----------------------------------------------");
    console.log("Next Nano Ledger application example started.");
    console.log("-----------------------------------------------");
